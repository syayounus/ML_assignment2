...
https://github.com/syayounus/ML\_assignment2.git
...
All models were trained and evaluated on the same train-test split with `random\_state=42`.
...
|   |-- knn.pkl
|   |-- naive\_bayes.pkl
|   |-- random\_forest\_ensemble.pkl
|   |-- feature\_columns.json
|   |-- target\_names.json
|   |-- evaluation\_reports.json
```
...
Then upload `test\_data.csv`, select any model from the dropdown, and review the metrics.
